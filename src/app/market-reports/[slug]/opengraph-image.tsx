...
export const alt = "Construction Capital Market Report";
...
export default async function Image({
  ...
  const report = MARKET_REPORTS.find((r) => r.slug === slug);
  const title = report?.title ?? "Market Report";
  const category = report?.category?.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) ?? "Analysis";
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div style={{ fontSize: "48px", fontWeight: 700, color: "#ffffff", lineHeight: 1.1, marginBottom: "20px", maxWidth: "900px" }}>
          {title}
        </div>
        <div style={{ fontSize: "22px", color: "rgba(255, 255, 255, 0.6)", lineHeight: 1.4, maxWidth: "700px" }}>
          Market intelligence for UK property developers
        </div>
